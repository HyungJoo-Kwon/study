...
class MyApp(QtWidgets.QDialog):
    def __init__(self):
        ...
        uic.loadUi('./qt/qt_test_copy.ui', self)
    ...
    def circleBtnClicked(self):
        if self.count != 0:
            ...
            img_rgb = cv2.cvtColor(self.img, cv2.COLOR_BGR2RGB)
            img_gray = cv2.cvtColor(self.img, cv2.COLOR_BGR2GRAY)
            _, img_gray = cv2.threshold(img_gray, 220, 255, cv2.THRESH_BINARY)

            circles= cv2.HoughCircles(img_gray, cv2.HOUGH_GRADIENT, 1, 150, param1 = 250, param2 = 10, minRadius = 35, maxRadius = 50)
            print(circles[0])
            print(circles[0][0])
            for i in circles[0]:
                
                cv2.circle(img_rgb, (i[0], i[1]), int(i[2]), (0, 0, 255), 5)
            outImg = img_rgb
            self.displayOutputImage(outImg,'dst')
        else:
            self.filename = "파일을 로드 하세요"
